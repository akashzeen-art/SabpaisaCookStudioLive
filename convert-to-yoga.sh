...
# Convert Food Theme to Cook Theme
# Run this script from the project root

echo "Converting project from Food theme to Cook theme..."

# Find all TypeScript/JavaScript files
find ./client -type f \( -name "*.tsx" -o -name "*.ts" -o -name "*.jsx" -o -name "*.js" \) -exec sed -i '' \
  -e 's/TheCookStudio/TheCookStudio/g' \
  ...
  -e 's/culinary/Cook/g' \
  -e 's/Cook/CookFlow/g' \
  -e 's/bg-food-cream/bg-Cook-cream/g' \
  -e 's/bg-food-brown/bg-Cook-purple/g' \
